...
if (isset($_POST["submit"])) {
    ...
    if (empty($name)) {
        ...
        if (isset($_FILES["image"]) && $_FILES["image"]["error"] != 4) {
            ...
            echo "<script>alert('ఏ చిత్రం అప్‌లోడ్ చేయబడలేదు');</script>";
        }
    }
}
?>
...
<body>
    <form action="" method="post" autocomplete="off" enctype="multipart/form-data" onsubmit="return validateForm()">
        ...
        <label for="image">చిత్రం:</label>
        <input type="file" name="image" id="image" accept=".jpg, .jpeg, .png" onchange="previewImage()"><br>
        
        <img id="imagePreview" src="#" alt="చిత్రం ప్రివ్యూ"><br><br>

        <button type="submit" name="submit">సమర్పించండి</button>
    </form>
